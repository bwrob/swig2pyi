...
option.setPricingEngine(
    ql.MCEuropeanEngine(
        process, "pseudorandom", timeSteps=1, requiredTolerance=0.02, seed=42
    )
)
value = option.NPV()
...
# %%
option.setPricingEngine(
    ql.MCEuropeanEngine(process, "lowdiscrepancy", timeSteps=1, requiredSamples=32768)
)
value = option.NPV()
...
# %%
df = pd.DataFrame(
    results, columns=["Method", "Option value", "Error estimate", "Actual error"]
)
