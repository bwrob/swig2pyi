...
def calibrate(
    model: ql.G2 | ql.HullWhite | ql.BlackKarasinski,
    ...
    method = ql.Simplex(l)
    model.calibrate(
        cast("list[ql.CalibrationHelper]", helpers),
        method,
        ql.EndCriteria(1000, 250, 1e-7, 1e-7, 1e-7),
    )

    totalError = 0.0
    data = []
    for swaption, helper in zip(swaptionVols, helpers, strict=False):
